...
namespace PublishingHouse.WebApi.Controller
{
    ...
    public class OrderStatusController(IOrderStatusService service, IMapper<IOrderStatus, OrderStatusRequestDto, OrderStatusResponseDto> mapper) : ControllerBase
    {
        [HttpGet]
        ...
        public async Task<IActionResult> Create([FromBody] OrderStatusRequestDto dto)
        {
            try
            {
        ...
        [Authorize(Roles = "Admin")]
        public async Task<IActionResult> Update([FromRoute] int id, [FromBody] OrderStatusRequestDto dto)
        {
            try
            {
